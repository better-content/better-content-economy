...
- The existing `better_content_economy:coin_purse` Curios storage remains seven coin-only slots and is shown as an attached 2x4 panel in the survival inventory (the eighth cell is intentionally inactive). The menu always reserves all seven slots and resolves their Curios storage dynamically, so the client and server retain the same slot layout while capability synchronization completes.
